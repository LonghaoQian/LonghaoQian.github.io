...
- System Ruby here is 3.0.2 (Ubuntu 22.04 default), but the committed `Gemfile.lock` was generated with Bundler 4.0.6, which requires Ruby >= 3.2. Forcing a resolve with an older Bundler (e.g. 2.5.x) silently downgrades `jekyll-sass-converter` to 2.2.0 (`sassc`/libsass), which cannot compile the theme's `color.channel()` Sass — that needs Dart Sass 1.79+, pinned here via `sass-embedded ~> 1.100`. Symptom: `Sass::CompileError: Undefined function.` on `_themes.scss` during `jekyll build`/`serve`.
- Given that mismatch, local preview on this machine uses the repo's Docker workflow instead of native Ruby: `docker compose up -d`, then check `curl -fsS http://127.0.0.1:8080/al-folio/`. The `ruby:slim` image satisfies the Bundler/Ruby version the lockfile expects, so nothing gets silently downgraded.
- If attempting the native Ruby route again anyway: this machine's Ruby install doesn't put `bundle` on PATH (Ubuntu's `ruby-full` package ships the bundler gem without a bin stub). Symlink it manually, e.g. `ln -sf "$(gem which bundler | xargs dirname)/../libexec/bundle" ~/.local/bin/bundle` (or the equivalent path for whichever bundler version is installed), and native gem extensions (nokogiri, ffi, bigdecimal, google-protobuf) need `ruby3.0-dev` + `build-essential` installed via apt first.
